...
class AbstractContext(object):
	...
	def getFunctionCache(self):
		if not self._function_cache:
			cache_size_mb = self.getSettings().getCacheSize()
			self._function_cache = FunctionCache(os.path.join(self._getCachePath(), 'cache'), max_file_size_kb=cache_size_mb * 1024)
		return self._function_cache

	def getSearchHistory(self):
		if not self._search_history:
			search_size = self.getSettings().getSearchSize()
			self._search_history = SearchHistory(os.path.join(self._getCachePath(), 'search'), search_size)
