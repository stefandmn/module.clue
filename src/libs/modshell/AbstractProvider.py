...
class AbstractProvider(object):
			...
			if re_match is not None:
				...
				if method is not None:
					...
					if not isinstance(result, tuple):
						result = result, {}
					return result
		raise ModuleException("Mapping for path '%s' not found" % path)
	...
	def _internal_search(self, context, re_match):
		...
		if command == 'remove':
			...
			query = params['q']
			result, new_query = context.getUI().onKeyboardInput(context.localize(30905, "Rename"), query)
			if result:
				context.getUI().showBusyDialog()
				try:
			...
			result, query = context.getUI().onKeyboardInput(context.localize(30903, "Search"))
			if result:
