...
class SearchHistoryItem(DirectoryItem):
	...
	def __init__(self, context, query, image=None, fanart=None):
		...
		context_menu = [(context.localize(30904, "Remove"), 'RunPlugin(%s)' % context.createUri(['media/search', 'remove'], params={'q': query})),
						(context.localize(30905, "Rename"), 'RunPlugin(%s)' % context.createUri(['media/search', 'rename'], params={'q': query})),
						(context.localize(30906, "Clear"), 'RunPlugin(%s)' % context.createUri(['media/search', 'clear']))]
