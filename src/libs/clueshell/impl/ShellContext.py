...
from .ShellPlaylist import ShellPlaylist
from .ShellPlayer import ShellPlayer
...
class ShellContext(AbstractContext):
	...
	def __init__(self, path='/', params=None, plugin_name=u'', plugin_id=u'', override=True):
		...
		self._settings = ShellSettings(self._addon)
	...
	def getSystemVersion(self):
		if not self._system_version:
			self._system_version = ShellVersion(version='', releasename='', appname='')
		return self._system_version

	def getVideoPlaylist(self):
		if not self._video_playlist:
			self._video_playlist = ShellPlaylist('video', weakref.proxy(self))
		return self._video_playlist

	def getAudioPlaylist(self):
		if not self._audio_playlist:
			self._audio_playlist = ShellPlaylist('audio', weakref.proxy(self))
		return self._audio_playlist

	def getVideoPlayer(self):
		if not self._video_player:
			self._video_player = ShellPlayer('video', weakref.proxy(self))
		return self._video_player

	def getAudioPlayer(self):
		if not self._audio_player:
			self._audio_player = ShellPlayer('audio', weakref.proxy(self))
		return self._audio_player

	def getUI(self):
		if not self._ui:
			self._ui = ShellContextUI(self._addon, weakref.proxy(self))
		return self._ui
	...
	def clone(self, new_path=None, new_params=None):
		...
		new_context = ShellContext(path=new_path, params=new_params, plugin_name=self._plugin_name, plugin_id=self._plugin_id, override=False)
